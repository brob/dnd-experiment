...
async function getSpells() {
    const spells = await axios.get('https://www.dnd5eapi.co/api/classes')
    ...
    return Promise.all(spellDetails)
        .then(values => {
            const sanityValues = values.map(value => {
                return {_id: value.index, _type: 'class', ...value}
                
            })
            
            console.log(sanityValues)
            fs.writeFileSync('classes.json', JSON.stringify(sanityValues));
